...
def input_batch_fn(df, batch_size=100, seed=None, num_epochs=1):
    """Input builder function."""

    all_continuous_cols = [tf.constant(df[k].values) for k in CONTINUOUS_COLUMNS]

    all_categorical_cols = [tf.SparseTensor(
        indices=[[i, 0] for i in range(df[k].size)],
        values=df[k].values,
        dense_shape=[df[k].size, 1])
    for k in CATEGORICAL_COLUMNS]

    all_labels = tf.constant(df[LABEL_COLUMN].values)

    continuous_col, categorical_col, label = \
        tf.train.slice_input_producer([all_continuous_cols,
                                       all_categorical_cols, all_labels],
    ...
    dataset_dict = {
        k : continuous_col[CONTINUOUS_COLUMNS.index(k)] for k in CONTINUOUS_COLUMNS
    }
    dataset_dict.update(
        {
            k : categorical_col[CATEGORICAL_COLUMNS.index(k)] for k in CATEGORICAL_COLUMNS
        }
    )
    dataset_dict.update(label)
    ...
    batch_labels = tf.string_to_number(batch_labels, out_type=tf.int32)
...
epochs= 5
batch_size = 1000
m.fit(input_fn=lambda: input_batch_fn(df_train, batch_size=batch_size, seed=42, num_epochs=epochs   ), steps=train_steps)
